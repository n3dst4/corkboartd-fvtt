...
const moduleOrSystem = "module";

const jsonText = (
  await fs.readFile(`./public/${moduleOrSystem}.json`)
).toString();

const { id: packageId } = JSON.parse(jsonText);
...
const preambleHtml =
  '\n<script type="module">\n' +
  preambleCode.replace("__PATH__", `/systems/${packageId}/@react-refresh`) +
  "\n</script>\n";
const headTag = "<head>";
...
const config = defineConfig(({ mode }) => {
  ...
  return {
    root: "src/",
    base: `/${moduleOrSystem}s/${packageId}/`,
